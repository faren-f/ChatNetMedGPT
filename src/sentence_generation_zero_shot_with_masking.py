...
def sentence_generation(edge_index, nodes, queried_node_types, queried_edge_types, test_ratio, val_ratio, param, seed, mask_ratio=.05):

    ## Train-val-test split of edge list of queried types    
    set_seed(seed)  
    rest_edges = edge_index.loc[~edge_index['relation'].isin(queried_edge_types)]    

    # Make one-sided edges (currently valid only for disease-drug ralations)
    edges_i = edge_index.loc[
            (edge_index['relation'] == queried_edge_types[0]) & (edge_index['node_types'] == queried_node_types[0]), 
    ...
    #### all the edges except for ['indication', 'contraindication', 'off-label use']
    ...
    edges_rest_remove = []
    for i in test_nodes:
        rest_edges_node_i = rest_edges.loc[rest_edges['x_index'] == i]
        N_remove = round(rest_edges_node_i.shape[0] * mask_ratio)
    
        if N_remove >= 0:
            edges_rest_remove.append(rest_edges_node_i.sample(n=N_remove))
    
    edges_rest_remove = pd.concat(edges_rest_remove, ignore_index=True)

    to_remove_xy = edges_rest_remove['xy'].astype(str)

    # make reversed "y|x"
    to_remove_xy_rev = (
        edges_rest_remove['y_index'].astype(str) + '|' +
        edges_rest_remove['x_index'].astype(str)
    )
    
    # union (unique) of both directions
    to_remove_edges_rest_bidir = pd.unique(pd.concat([to_remove_xy, to_remove_xy_rev], ignore_index=True))
    ...
    to_remove_pretrain = [f'{x}|{y}' for x,y in positive_edges_val_test.T]
    print(len(to_remove_pretrain))
    # to_remove_pretrain = to_remove_pretrain + list(edges_rest_remove['xy'])
    to_remove_pretrain = to_remove_pretrain + list(to_remove_edges_rest_bidir)
