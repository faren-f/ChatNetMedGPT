...
def sentence_generation(edge_index, nodes, queried_node_types, queried_edge_types, area, area_nodes, val_ratio, relation_node_types, param, seed):
    ...
    for r, (i, (node_source1, node_source2)) in enumerate(queried_dict.items()):
        ...
        #seperates all the nodes from relation r and node type is disaese|drug
        all_nodes_r = edge_index.loc[(edge_index['relation'] == i) & (edge_index['node_types'] == queried_node_types[r]), 'x_index'].unique()

        # find the intersected nodes of that relation with area_nodes_i
...
##########
    ...
    #### debugging
    edges_A = set(zip(train_data_edge_index_pretrain[0].tolist(), train_data_edge_index_pretrain[1].tolist()))
